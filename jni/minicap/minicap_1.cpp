...
void rgbToNV12_libyuv(const Minicap::Frame& frame,
                       uint8_t* dst,
                       int width, int height)
{
    uint8_t* yPlane  = dst;
    ...
    std::vector<uint8_t> i420Buf(width*height*3/2);
    uint8_t* yI420 = i420Buf.data();
    uint8_t* uI420 = yI420 + width*height;
    uint8_t* vI420 = uI420 + (width/2)*(height/2);

    if (frame.format == Minicap::FORMAT_BGRA_8888) {
        libyuv::BGRAToI420(
            (const uint8_t*)frame.data, frame.stride * frame.bpp,
            yI420, width,
            uI420, width/2,
            vI420, width/2,
            width, height
        );
    } else if (frame.format == Minicap::FORMAT_RGBA_8888) {
        libyuv::RGBAToI420(
            (const uint8_t*)frame.data, frame.stride * frame.bpp,
            yI420, width,
            uI420, width/2,
...
while (true) {
    ...
    rgbToNV12_libyuv(frame, yuvBuf.data(),
                 desiredInfo.width,
                 desiredInfo.height);
